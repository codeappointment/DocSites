...
public class ForumNotificationAdapter extends RecyclerView.Adapter<ForumNotificationAdapter.VHolder> {
    ...
    public void onBindViewHolder(@NonNull final ForumNotificationAdapter.VHolder holder, final int position) {
        ...
        Glide.with(context.getApplicationContext()).load("https://graph.facebook.com/" + users_sources.get(position) + "/picture?width=800").into(holder.userSource);
    }
    ...
    class VHolder extends RecyclerView.ViewHolder {
        ...
        private VHolder(View itemView) {
            super(itemView);
            notifiationText = itemView.findViewById(R.id.noti_text);
            time = itemView.findViewById(R.id.notify_time);
            del = itemView.findViewById(R.id.del_notification);
            userSource = itemView.findViewById(R.id.user_source_image);
