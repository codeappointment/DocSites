...
//        try {
        ...
        if (doc_name.get(position).equals(" ")) {
            holder.docText.setVisibility(View.GONE);
        }
        if (doc_text.get(position).length() > 300) {
            String text = "<font color=#b4b4b4> ...continue reading</font>";
            Scanner sc = new Scanner(doc_text.get(position));
            String short_Text = sc.nextLine() + "<br/>" +
                    sc.nextLine() + "<br/>" +
                    sc.nextLine() + "<br/>" +
                    sc.nextLine() + "<br/>" +
                    sc.nextLine() + text;
            holder.docText.setText(Html.fromHtml(short_Text), TextView.BufferType.SPANNABLE);
        } else {
        ...
        try {
            ...
            if (reply_preview.get(position).equals("blank")) {
                ...
                if (reply_preview.get(position).length() > 100) {
                    String previewText=reply_preview.get(position).substring(0, 100) + "....";
                    holder.preview_reply.setText(previewText);
                } else if (reply_preview.get(position).equals(" ")) {
                    holder.preview_reply.setVisibility(View.GONE);
                } else {
        ...
        try {
            Glide.with(context.getApplicationContext()).load("https://graph.facebook.com/" + user_id.get(position) + "/picture?width=800").into(holder.pic);

            Glide.with(context.getApplicationContext()).load(postImageUrl.get(position)).into(holder.postImage);
            if (postImageUrl.get(position).equals("blank")) {
                holder.progressBar.setVisibility(View.GONE);
            }
            if (!replyImageUrl.get(position).equals("blank")) {
                Glide.with(context.getApplicationContext()).load(replyImageUrl.get(position)).into(holder.replyImage);
        ...
        holder.report.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(context);
                AlertDialog dialog = builder.create();
                dialog.setButton(DialogInterface.BUTTON_POSITIVE, "Report Post", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                ...
                dialog.setButton(DialogInterface.BUTTON_NEUTRAL, "Report ID", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        final HashMap<String, Object> reportList = new HashMap<>();
                        reportList.put(myID, myName);
...
//        AdView mAdView;
        ...
        private VHolder(View itemView) {
            ...
            docText =  itemView.findViewById(R.id.user_text);
            postTime =  itemView.findViewById(R.id.time);
            delete_post =  itemView.findViewById(R.id.delete_post);
            preview_reply =  itemView.findViewById(R.id.reply_preview_text);
            preview_reply_name =  itemView.findViewById(R.id.reply_preview_name);
            varified =  itemView.findViewById(R.id.varified);
            varified_reply =  itemView.findViewById(R.id.varified_reply);
            ...
            preview_pic =  itemView.findViewById(R.id.reply_preview_image);
            progressBar =  itemView.findViewById(R.id.forum_layout_progressbar);
            relativeLayout =  itemView.findViewById(R.id.forum_layouts);
        }
    }

    private void myToast() {
        Toast toast = makeText(context, "Post deleted", Toast.LENGTH_SHORT);
        toast.setGravity(Gravity.CENTER, 0, 0);
        toast.show();
